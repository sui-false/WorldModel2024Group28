...
async ([parent_bid, bid_attr_name, tags_to_mark]) => {
    ...
    let intersection_observer = new IntersectionObserver(
        entries => {
          entries.forEach(entry => {
            ...
            elem.setAttribute('visFlg', Math.round(entry.intersectionRatio * 100) / 100);
    ...
    while (i < elements.length) {
        ...
        elem.setAttribute("action_marks", "0");
            ...
            if (set_of_marks_tags.has(elem.tagName.toLowerCase()) || (elem.onclick != null) || (window.getComputedStyle(elem).cursor == "pointer")) {
                ...
                if (area >= 20) {
                    ...
                    if (som_buttons.every(button => !button.contains(elem))) {
                        ...
                        if (!(parent && parent.tagName.toLowerCase() == "span" && parent.children.length === 1 && parent.getAttribute("role") && parent.getAttribute("action_marks") === "1")) {
                            // all checks have passed, flag the element for inclusion in set-of-marks
                            elem.setAttribute("action_marks", "1");
                            ...
                            while (parent) {
                                if (parent.getAttribute("action_marks") === "1") {
                                    parent.setAttribute("action_marks", "0")
                                }
